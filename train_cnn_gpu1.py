...
IS_COLOR = True
NOISE_LV = 0.5
COMMENT = 'no tv - Resblock ORIGINAL' #Kasih judul percobaan disini
GPU_ID = 1
...
def main(checkpoint=None, batch_size=32, lr=0.001):
    ...
    net = DenoiserNetORG(IS_COLOR, NOISE_LV, NUM_FEATS, DEPTH, KERNEL_SIZE, COMMENT)
    net_name = net.name
...
def test(checkpoint=None):
    ...
    test_set = ImageDataset('bsd300', IS_COLOR, NOISE_LV)
    ...
    with torch.no_grad():
        demo(net, test_dir, checkpoint, test_loader, save=True)
    pass
